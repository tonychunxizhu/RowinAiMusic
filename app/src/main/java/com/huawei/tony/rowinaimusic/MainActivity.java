...
public class MainActivity extends AppCompatActivity {
    ...
    String prompt;
    String id0, id1,id;
    String videoUrl0,  videoUrl1, videoUrl;

    String audeoUrl0, audeoUrl1, audioUrl;
    String status;
    String title, lyric;
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            String s = genMusic(prompt);
                            Log.d(TAG,"Return from genMusic "+s);
                            reply.setText(title+"\n"+lyric);
                            playMusic(s);
    ...
    void playMusic(String video) throws InterruptedException {
//        videoUrl = "https://cdn1.suno.ai/6b57f41f-bb9d-4d83-8655-4e17950953a3.mp4";
//        title ="夏天";
//        String theVideoUrl = videoUrl;
        Log.d(TAG,"in playMusic "+video);
        String destFileDir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Download";
        String destFileName = "video.mp4";
        //File backup = new File(destFileDir+"/"+title.trim()+".mp4");
        DownloadUtil.get().download(video, destFileDir, destFileName, new DownloadUtil.OnDownloadListener() {
            @Override
     ...
     */
    public native String nativeGetMusicById(String id);
    ...
    public String genMusic(String prompt) throws InterruptedException {
...
//        id = id0; //two musics are generated by SUNO by default, select the first music
        id = "3650720d-52dc-4a61-a66f-50c9c8257902";
        ret = nativeGetMusicById(id);
        //wait until the music completes, should already checked in native code
        do {
            status = jsonData.getMusicItems(ret).get(0).status;
            Log.d(TAG,"STATUS from nativeGetMusicById: "+status);
            ret = nativeGetMusicById(id);
            Thread.sleep(1000);
        ...
        //for test
        audioUrl=jsonData.getAudioUrlByID(ret);
